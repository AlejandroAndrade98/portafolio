...
function RadialIntro({
  ...
  // ✅ ID estable: usa id si existe, si no usa name, si no usa i
  const stableId = item.id ?? item.label ?? i;
  ...
  return (
    <motion.div
      ...
      data-angle={i * step}
      layoutId={`arm-${stableId}`}
    >
    <motion.img
      data-arm-image
      className="rounded-full object-fill absolute left-1/2 top-1/2 aspect-square -translate-x-1/2 -translate-y-1/2 will-change-transform invert brightness-200"
      style={{
